document.addEventListener('DOMContentLoaded', function() {
    console.log('🚀 Advanced Portfolio Enhancement Loading...');
    ...
    function getCachedDescription(repoName) {
        const cached = localStorage.getItem(getCacheKey(repoName));
        return cached ? JSON.parse(cached) : null;
    }

    function setCachedDescription(repoName, data) {
        localStorage.setItem(getCacheKey(repoName), JSON.stringify({
            ...data,
            timestamp: Date.now()
        }));
    }
    ...
    function getCachedSkills() {
        const cached = localStorage.getItem(`portfolio_skills_${PORTFOLIO_VERSION}`);
        return cached ? JSON.parse(cached) : null;
    }

    function setCachedSkills(skills) {
        localStorage.setItem(`portfolio_skills_${PORTFOLIO_VERSION}`, JSON.stringify({
            skills: skills,
            timestamp: Date.now()
        }));
    }
    ...
    function clearOldCache() {
        ...
        for (let i = 0; i < localStorage.length; i++) {
            ...
            if (key.startsWith('portfolio_') && !key.includes(PORTFOLIO_VERSION)) {
    ...
    async function generateAIDescription(repoData) {
        // Use smart enhancement for now (HF API often has issues)
    ...
    function createSmartEnhancement(repoData) {
        ...
        // Add project context based on detected technologies
    ...
    async function enhanceProjectDescriptions() {
        ...
        if (projectsToEnhance.length > 0) {
            ...
            for (let { project, repoInfo, descriptionElement } of projectsToEnhance) {
                ...
                descriptionElement.innerHTML = `${originalText} <em style="color: #007bff;">🔄 Analyzing...</em>`;
        ...
        updateSkillsSection(allDetectedSkills);
    }

    // ================== ADVANCED SKILLS SECTION ==================
    function updateSkillsSection(detectedSkills) {
        setTimeout(() => {
            const cachedSkills = getCachedSkills();
            if (cachedSkills && detectedSkills.size === 0) {
                console.log('💾 Using cached skills');
                displaySkills(cachedSkills.skills);
                return;
            }
        ...
        }, detectedSkills.size > 0 ? 10000 : 100);
    }
    ...
    // Start enhancement
    enhanceProjectDescriptions().then(() => {
        console.log('🎉 All project enhancements completed!');
    }).catch(error => {
        console.error('❌ Enhancement failed:', error);
    });
    
    console.log('🎉 Advanced Portfolio Enhancement Loaded Successfully');
});
